...
orig <- reshape2::dcast(orig, subj+item+cond+region ~ fixationtype, value.var = "value")
...
if(0){
## testing:
cols<-colnames(subj_re)
for(i in 1:40){
  print(cols[i])
}

for(i in 1:40){
  print(cols[i+ru_id*nsubj])
}

for(i in 1:40){
  print(cols[i+au_id*nsubj])
}
}

for(i in 1:nsubj){
subjdiff_ru[i,] <- exp(alpha + subj_re[,i]  + (beta_ru+subj_re[,i+ru_id*nsubj])/2) - exp(alpha + subj_re[,i] - (beta_ru+subj_re[,i+ru_id*nsubj])/2)
subjdiff_au[i,] <- exp(alpha + subj_re[,i]  + (beta_au+subj_re[,i+au_id*nsubj])/2) - exp(alpha + subj_re[,i] - (beta_au+subj_re[,i+au_id*nsubj])/2)
}
...
plot_auDillonE1<-mcmc_intervals(subjdiff_au,prob_outer = 0.95, point_est="mean")+
  ...
  theme(axis.title.y = element_text(angle = 90))+
  ggtitle("Dillon et al 2013 Expt 1: \n Ungrammatical \n agreement")+
  annotate("segment", x=ACTRpredictionmismatch[1], xend=ACTRpredictionmismatch[3], y=45, yend=45,arrow=arrow(ends="both", angle=90, length=unit(.2,"cm")),color="white")+
  annotate("segment", x=ACTRpredictionmismatch[1], xend=ACTRpredictionmismatch[3], y=44, yend=44,arrow=arrow(ends="both", angle=90, length=unit(.2,"cm")))+
  annotate("text", x=-30, y=43, label="Model prediction",family="serif",
                 fontface="bold", colour="darkred", size=3)+
  annotate("segment", x=meanau_quantile[1], xend=meanau_quantile[2], y=41, yend=41,arrow=arrow(ends="both", angle=90, length=unit(.2,"cm")))+
  annotate("text", x=-70, y=41.7, label="Mean effect",family="serif",fontface="bold", colour="darkred", size=3)
